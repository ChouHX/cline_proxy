...
import { getAccounts, saveAccounts, testAccount } from '../api/client';
import type { Account, AccountsResponse } from '../api/types';
...
export default function AccountsPage() {
  ...
  const { data, loading, error, reload } = useAsyncData<AccountsResponse>(() => getAccounts(), [token]);
  ...
  useEffect(() => {
    if (!data) return;
    setDraft(data.accounts.map((a) => ({ ...a })));
    setMode(data.mode);
